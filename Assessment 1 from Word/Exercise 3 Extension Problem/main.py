...
def save_data(filename, students):  # Function to save all students to file
...
def format_student(student):  # Function to format student data for display
...
def refresh_dropdown(): # Refresh the dropdown menu with current student names, added with the addition of editing features
...
def sort_students(): # Function to sort students by overall percentage
    if not students:
...
def add_student(): # Function to add a new student
    ...
    def save_new(): # Save new student data
        try:
...
def delete_student(): # Function to delete a student
    selected = student_var.get()
    if not selected:
...
def update_student(): # Function to update a student's details
    selected = student_var.get()
    if not selected:
        ...
        if s['name'] == selected:
            ...
            def save_update():
                try:
                    ...
                    s['grade'] = ('A' if s['percent']>=70 else 'B' if s['percent']>=60 else 'C' if s['percent']>=50 else 'D' if s['percent']>=40 else 'F')
                    save_data("studentMarks.txt", students)
                    refresh_dropdown()
                    popup.destroy()
                    output_box.delete(1.0, tk.END)
...
ttk.Button(btn_frame, text="Add", command=add_student, width=12).grid(row=1,column=1,padx=5,pady=5)
